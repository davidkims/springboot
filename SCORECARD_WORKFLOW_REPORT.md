# Scorecard Supply-Chain Security 워크플로우 보고서

이 문서는 `.github/workflows/scorecard.yml`에 정의된 워크플로우의 동작을 설명합니다. 해당 워크플로우는 브랜치 보호 규칙 변경, 주간 스케줄, `main` 브랜치 푸시, 수동 실행 시 트리거됩니다.

## 주요 기능
- OpenSSF Scorecard를 실행해 결과를 `results.sarif` 파일로 저장합니다.
- 결과를 아티팩트로 업로드하고 GitHub 코드 스캐닝 대시보드에 게시합니다.
